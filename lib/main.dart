...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'SahamAja'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<int> addSahamAja() async {
    Sahaminaja tlkm = Sahaminaja(ticker: "TLKM", open: 3380, high: 3500, last: 3490, change: "2,05");
    Sahaminaja ammn = Sahaminaja(ticker: "AMMN", open: 6750, high: 6750, last: 6500, change: "-3,7");
    Sahaminaja bren = Sahaminaja(ticker: "BREN", open: 4500, high: 4610, last: 4580, change: "1,78");
    Sahaminaja cuan = Sahaminaja(ticker: "CUAN", open: 5200, high: 5525, last: 5400, change: "3,85");

    List<Sahaminaja> listOfSahamAja = [tlkm, ammn, bren, cuan];

    return await this.handler.insertSahaminaja(listOfSahamAja);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        Sahaminaja newSahaminaja = Sahaminaja(
                        ...
                        await handler.insertSahaminaja(newSahaminaja);
          ...
          Expanded(
            child: FutureBuilder<List<Sahaminaja>>(
              future: handler.retrieveSahamAja(),
              builder: (BuildContext context, AsyncSnapshot<List<Sahaminaja>> snapshot) {
                if (snapshot.hasData) {
                  return ListView.builder(
                    itemCount: snapshot.data?.length,
                    itemBuilder: (BuildContext context, int index) {
                      Sahaminaja saham = snapshot.data![index];
                      return ListTile(
